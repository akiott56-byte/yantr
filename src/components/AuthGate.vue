...
import { Fingerprint, KeyRound, LoaderCircle, LockKeyhole, ShieldCheck, UserRound } from 'lucide-vue-next'
...
async function submit() {
  ...
  try {
    if (isSetup.value) {
      ...
      success(t('authGate.messages.setupComplete'))
    } else {
      await loginYantr({ username: username.value, password: password.value, pin: pin.value })
      success(t('authGate.messages.loginComplete'))
    }
    password.value = ''
    pin.value = ''
  } catch (error) {
...
<template>
  <div class="min-h-screen bg-(--bg-body) text-(--text-primary)">
    <div class="mx-auto flex min-h-screen w-full max-w-6xl flex-col justify-center gap-6 px-4 py-8 sm:px-6 lg:grid lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)] lg:items-center lg:px-8">
      ...
      <aside class="animate-fadeIn rounded-[28px] bg-(--surface-muted) px-6 py-7 sm:px-8 sm:py-9" style="animation-delay: 70ms">
        ...
        <div class="mt-6 space-y-3">
          <div v-for="item in guidance" :key="item" class="flex items-start gap-3 rounded-2xl bg-(--surface) px-4 py-4 smooth-shadow">
